<script lang="ts">
  ...
  onMount(async() => {
   ...
   const handleKeydown = (event: KeyboardEvent) => {
    ...
    const user: User | undefined = await store.get<User>("user");
    // check user
    ...
    if (
      currentUser.machine &&
      currentUser.user &&
      currentUser.machine === user?.machine &&
      currentUser.user === user?.user
    ) {
      await store.set("user", { machine: user.machine, user: user.user });
    }
    ...
    if (!source_valid) {
      ...
      if (result) {
        ...
        await message("没有选择远程数据源,无法使用本软件，软件将退出！");
      }
...
<div class="page-container">
  <div class="fixed-header">
   <div class="brand">
    ...
    <div class="month-nav">
      <button class="tooltip-container" aria-label="previous_month" onclick={() => calendar.changeMonth(-1)}>
        <span class="tooltip">上个月</span>
        <svg class="logo" style="fill: #94a3b8;" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" width="200" height="200"><path d="M629.291 840.832l60.331-60.331-268.501-268.501 268.501-268.501-60.331-60.331-328.832 328.832z"></path></svg>
      </button>
      <input
        type="month"
